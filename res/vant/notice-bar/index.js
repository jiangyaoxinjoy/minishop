...
VantComponent({
  ...
  watch: {
    text: function text() {
      // this.init() //获取不到节点，因为页面初始化未完成
      //所以在setData之后回调可以确保拿到页面节点
  ...
  methods: {
    init: function init() {
      var _this = this;
      // console.log(this.getRect('.van-notice-bar__content'));
      this.getRect('.van-notice-bar__content').then(function (rect) {
        ...
        _this.getRect('.van-notice-bar__content-wrap').then(function (rect) {
          ...
          // console.log(scrollable && wrapWidth < width);
